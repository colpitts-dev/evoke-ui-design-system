...
- `packages/@evoke-ui/core`: Core React components
- `packages/@evoke-ui/utils`: Shared React utilities
- `packages/@evoke-ui/tsconfig`: Shared `tsconfig.json`s used throughout the Turborepo
- `packages/eslint-preset-evoke-ui`: ESLint preset
...
For `evoke-ui-core`, the `build` command is the following:

```bash
tsup src/index.tsx --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `evoke-ui-core` then instructs the consumer to select the correct format:

```json:evoke-ui-core/package.json
{
  "name": "@evoke-ui/core",
  "version": "0.0.0",
  "main": "./dist/index.js",
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `evoke-ui-core/dist` which contains the compiled output.
...
Each file inside of `evoke-ui-core/src` is a component inside our design system. For example:

```tsx:evoke-ui-core/src/Button.tsx
...
```tsx:evoke-ui-core/src/index.tsx
import * as React from "react";
...
- Support using module path aliases like `@evoke-ui-core` for imports
...
Turborepo runs the `build` script for all publishable packages (excluding docs) and publishes the packages to npm. By default, this example includes `evoke-ui` as the npm organization. To change this, do the following:

- Rename folders in `packages/*` to replace `evoke-ui` with your desired scope
- Search and replace `evoke-ui` with your desired scope
- Re-run `yarn install`
